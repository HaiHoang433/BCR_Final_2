...
uint8_t image_data[32][32][3]; // RGB image data array
uint8_t current_label; // Current image label
uint32_t current_image_index = 0; // Current image index
uint32_t total_images_read = 0; // Total images read so far
float confidence;

const uint32_t NUMBER_OF_FIRST_IMAGES = 1000;
uint32_t number_of_true_predicted = 0;
...
uint32_t cifar10_classify(uint8_t image[32][32][3], float *confidence);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	    ...
	    */
	    ...
	    uint32_t predicted_label = cifar10_classify(image_data, &confidence);
